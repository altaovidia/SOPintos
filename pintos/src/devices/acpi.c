...
unsigned int *check_RSDPtr(unsigned int *ptr);
unsigned int *get_RSDPtr(void);
int check_header(unsigned int *ptr, char *sig);
...
int acpi_init(void)
{
   unsigned int *ptr = get_RSDPtr();

   // check if address is correct  ( if acpi is available on this pc )
   if (ptr != NULL && check_header(ptr, "RSDT") == 0)
   {
      ...
      while (0<entrys--)
      {
         ...
         if (check_header((unsigned int *) *ptr, "FACP") == 0)
         {
            entrys = -2;
            struct FACP *facp = (struct FACP *) *ptr;
            if (check_header((unsigned int *) facp->DSDT, "DSDT") == 0)
            {
